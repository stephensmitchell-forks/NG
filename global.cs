...
//ORIGINAL LINE: class DLL_HEADER multithreadt
  public class multithreadt
  {
	...
	public string tcl_todo = new string(""); // tcl commands set from parallel thread
	public multithreadt()
	{
	  pause = 0;
	  GlobalMembers.testmode = 0;
	  redraw = 0;
	  drawing = 0;
	  terminate = 0;
	  running = 0;
...
namespace netgen
{
  ...
  public class TraceGlobal : System.IDisposable
  {
	private string name;
	public TraceGlobal(string _name)
	{
		this.name = _name;
		Console.Write("init global ");
		Console.Write(name);
		Console.Write("\n");
	}
	public void Dispose()
	{
		Console.Write("exit global ");
		Console.Write(name);
		Console.Write("\n");
	}
  }
